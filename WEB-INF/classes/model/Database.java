...
public class Database{
	...
	public User getUser(String emailid,Connection conn){
		String sql="select user_id,password,name,role_id,avatar from user_details where email=?";
		User user=new User();
		try{
			PreparedStatement pstmt=conn.prepareStatement(sql);
			pstmt.setString(1,emailid.toLowerCase());
			...
			user.setUserAvatar(rs.getBytes(5));
		}
		catch(SQLException e){
			System.out.println(e.getMessage());
		}
...
//for web app
	public User getUser(int userID,Connection conn){
		String sql="select email,name,role_id,avatar from user_details where user_id=?";
		User user=new User();
		try{
			PreparedStatement pstmt=conn.prepareStatement(sql);
			...
			ResultSet rs=pstmt.executeQuery();
			rs.next();
			user.setUserRole(rs.getInt(3));
			user.setUserId(userID);
			user.setUserName(rs.getString(2));
			user.setUserEmail(rs.getString(1));
			user.setUserAvatar(rs.getBytes(4));
		}
		catch(SQLException e){
	...
	public boolean registerUser(User user,Connection conn){
		String sql="insert into user_details(email,password,name,role_id,created_date,avatar) values(?,?,?,?,?,?)";
		boolean status=false;
		try{
			PreparedStatement pstmt=conn.prepareStatement(sql);
	...
	public User signInUser(String emailid,String password,Connection conn){
		String sql="select user_id,role_id,name,avatar from user_details where email=? AND password=?";
		int userID=-1;
		int userType=1;		
		try{
			PreparedStatement pstmt=conn.prepareStatement(sql);
			...
			while(rs.next()){
				...
				User user=new User(rs.getInt(1),rs.getInt(2),rs.getString(3),rs.getBytes(4));
				return user;
			}
		}
		catch(SQLException e){
	...
	/*public int getUserType(int user_id,Connection conn){
		String sql="select role_id from user_details where user_id=?";
		int role_id=-1;
		try{
			PreparedStatement pstmt=conn.prepareStatement(sql);
	...
	public boolean verifyCategory(Category category,Connection conn){
		String sql="select category_id from category where user_id=? AND category_name ilike ?";
		try{
			PreparedStatement pstmt=conn.prepareStatement(sql);
	...
	public ArrayList<Category> getUserCategories(int auth_id,Connection conn){
		...
		String sql="SELECT category_id,category_name,description FROM category WHERE user_id=? ORDER BY category_id";
		try{
			PreparedStatement pstmt=conn.prepareStatement(sql);
	...
	public ArrayList<Category> getUserCategoriesName(int auth_id,Connection conn){
		ArrayList<Category> categories=null;
		String sql="SELECT category_id,category_name FROM category WHERE user_id=? ORDER BY category_name";
		try{
			PreparedStatement pstmt=conn.prepareStatement(sql);
			pstmt.setInt(1,auth_id);
			ResultSet rs=pstmt.executeQuery();
			...
			while(rs.next()){
				Category category=new Category(rs.getInt(1),auth_id,rs.getString(2),"");
				categories.add(category);
			}
		}
		catch(SQLException e){
	...
	public Category getCategory(int categ_id,int user_id,Connection conn){
		String sql="select category_name,description from category where category_id=? and user_id=?";
		...
		try{
			...
			while(rs.next()){
				...
				category.setCategoryDesc(rs.getString(2));
	...
	public boolean editCategory(Category category,Connection conn){
		String sql="update category set category_name=?,description=? where category_id=? and user_id=?";
		boolean status=false;
		try{
			PreparedStatement pstmt=conn.prepareStatement(sql);
			...
			pstmt.setInt(4,category.getAuthId());
			int affectedRows=pstmt.executeUpdate();
			if(affectedRows==1){
				status=true;
			}
	...
	public boolean deleteCategory(int categoryId,int user_id,Connection conn){
		String sql1="update post set category_id=? where category_id=? AND user_id=?";
		String sql2="delete from category where category_id=? AND user_id=?";
		boolean status=false;
		try{
			PreparedStatement pstmt=conn.prepareStatement(sql1);
			pstmt.setInt(1,0);
